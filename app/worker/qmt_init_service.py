...
注意：QMT 不提供新闻数据，因此初始化流程中不包含新闻同步步骤
"""
import asyncio
import logging
...
class QMTInitService:
    ...
    async def initialize(self):
        ...
        self.sync_service = await get_qmt_sync_service()
        logger.info("✅ QMT初始化服务准备完成")
    ...
    async def run_full_initialization(
        ...
        try:
            ...
            logger.info(f"🎉 QMT数据初始化完成！耗时: {duration:.2f}秒")

            return self._get_initialization_summary()

        except Exception as e:
            logger.error(f"❌ QMT数据初始化失败: {e}")
    ...
    async def _step_initialize_basic_info(self):
        ...
        if result:
            self.stats.basic_info_count = result.get("inserted", 0) + result.get("updated", 0)
    ...
    async def _step_initialize_historical_data(self, historical_days: int):
        ...
        if result:
            self.stats.historical_records = result.get("records_synced", 0)
    ...
    async def _step_initialize_financial_data(self):
        ...
        try:
            ...
            if result:
                self.stats.financial_records = result.get("records_synced", 0)
    ...
    async def _step_initialize_quotes(self):
        ...
        try:
            ...
            if result:
                self.stats.quotes_count = result.get("updated", 0)
    ...
    async def _step_verify_data_integrity(self):
        ...
        logger.info(f"    股票基础信息: {basic_count}条")
        logger.info(f"    扩展字段覆盖: {extended_count}条 ({extended_count/basic_count*100:.1f}%)" if basic_count > 0 else "    无基础数据")
        logger.info(f"    行情数据: {quotes_count}条")
...
# 全局初始化服务实例
_qmt_init_service = None

async def get_qmt_init_service() -> QMTInitService:
    """获取QMT初始化服务实例"""
    global _qmt_init_service
    if _qmt_init_service is None:
        _qmt_init_service = QMTInitService()
        await _qmt_init_service.initialize()
    return _qmt_init_service


# APScheduler兼容的初始化任务函数
async def run_qmt_full_initialization(
    historical_days: int = 365,
    skip_if_exists: bool = True
):
    """APScheduler任务：运行完整的QMT数据初始化"""
    try:
        ...
        logger.info(f"✅ QMT完整初始化完成: {result}")
        return result
    except Exception as e:
        logger.error(f"❌ QMT完整初始化失败: {e}")
        raise